...
mCore.Draw3DText = (function(x, y, z, text, r, g, b, useScale, font)
...
---@alias ValidFonts "RobotoRegular" | "BebasNeueOtf" | "FontAwesome"
local validFonts = {
     RobotoRegular = true,
     BebasNeueOtf  = true,
     FontAwesome   = true,
...
local iconTxd = CreateRuntimeTxd("mCoreIcons")
function GetIcon(name)
     ...
     if not loadedIcons[name] then
          ...
          local imagePath = ("icons/%s.png"):format(iconName)
          local success = CreateRuntimeTextureFromImage(iconTxd, textureName, imagePath)

          if not success then
               return print(("^7[^3mCore^7]: ^1Err:^7 Failed to create texture from image ^6'%s'^7."):format(imagePath))
          end

          local resolution = GetTextureResolution("mCoreIcons", textureName)
          if resolution.x == 0 and resolution.y == 0 then
               return print(("^7[^3mCore^7]: ^1Err:^7 Texture resolution is invalid for ^6'%s'^7."):format(textureName))
          end
...
local function Rotate3D(v, rot)
     ...
     local x        = v.x * (cosYaw * cosRoll) + v.y * (cosYaw * sinRoll) + v.z * (-sinYaw)
     local y        = v.x * (sinPitch * sinYaw * cosRoll - cosPitch * sinRoll)
         + v.y * (sinPitch * sinYaw * sinRoll + cosPitch * cosRoll)
         + v.z * (sinPitch * cosYaw)
     local z        = v.x * (cosPitch * sinYaw * cosRoll + sinPitch * sinRoll)
         + v.y * (cosPitch * sinYaw * sinRoll - sinPitch * cosRoll)
         + v.z * (cosPitch * cosYaw)
...
mCore.DrawCustomIcon = function(center, width, height, icon, rot, dir, color, faceCamera)
     if not center or type(center) ~= "vector3" then
          return print(("[^1Err]^0 Expected vector3 got '%s'"):format(type(center)))
     end

     local r, g, b, a = 255, 255, 255, 255
     if color then
          r, g, b, a = color[1] or 255, color[2] or 255, color[3] or 255, color[4] or 255
     end
     ...
     if type(icon) == "string" then
          iconData = mCore.RequestIcon(icon)
     elseif type(icon) == "table" then
          iconData = icon
     end

     if not iconData then
          return print(("[^1Err]^0 No icon data with key: '%s'"):format(icon))
     end

     if faceCamera then
          local playerPos = GetEntityCoords(PlayerPedId())
          dir = playerPos - center
     end

     dir = dir or vec3(0.0, 1.0, 0.0)
     dir = dir / #(dir)

     local worldUp = vec3(0.0, 0.0, 1.0)
     local right = crossProd(worldUp, dir)
     right = right / #(right)

     local up = crossProd(dir, right)
     up = up / #(up)

     rot = rot or vec3(0.0, 0.0, 0.0)
     right = Rotate3D(right, rot)
     up = Rotate3D(up, rot)
     ...
     local points = {
          center + right * -halfW + up * halfH,  -- top-left
          center + right * halfW + up * halfH,   -- top-right
          center + right * -halfW + up * -halfH, -- bottom-left
          center + right * halfW + up * -halfH,  -- bottom-right
     }
