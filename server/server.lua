...
mCore.RequestWebhook = (function(webhookName)
    while not webhooksloaded do Wait(200) end
...
mCore.getPlayer = (function(pid)
    local idf = GetPlayerIdentifierByType(pid, "license"):sub(9)

    local xPlayer = PLAYERS[pid]

    if not xPlayer then
        mCore.error(("[getPlayer]: No cached xPlayer found for player: %s(%s)"):format(GetPlayerName(pid), pid))
        return false
    end
    ...
    local playerDiscord = mCore.GetDiscordAwait(pid)

    if playerDiscord then
        Player.discord = playerDiscord
        Player.DiscordId = playerDiscord.id
    end

    Player.idf        = idf
    Player.identifier = idf
    Player.RPName     = xPlayer.getName() or "*mh* RP Name not found"
    Player.Name       = GetPlayerName(pid) or "*mh* Not Found"

    Player.Accounts   = xPlayer.getAccounts()

    if GetResourceState("mate-vipsystem") == "started" then
        local has, lvl = mCore.getVIPUserAsync(pid)
